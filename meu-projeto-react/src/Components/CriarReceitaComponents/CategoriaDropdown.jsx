...
export default function CategoriaDropdown({ categoriasSelecionadas, setCategoriasSelecionadas }) {
  ...
  return (
    <div className="relative inline-block text-left flex">
      <div className="border border-[#c9c9c9] px-3 py-1 rounded-md cursor-pointer bg-white hover:border-[#FF7B00]" onClick={() => setAberto(!aberto)}>
      ...
      {aberto && (
        <div className="absolute z-10 mt-1 w-60 rounded-md bg-white shadow-lg border border-gray-200 p-2 max-h-60 overflow-y-auto">
          {categoriasDisponiveis.map((categoria) => {
            ...
            return (
              <div key={categoria} onClick={() => toggleCategoria(categoria)} className={`cursor-pointer text-sm px-3 py-2 rounded mb-1 ${
                  selecionada
