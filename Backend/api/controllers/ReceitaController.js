...
module.exports = {
    ...
    create: async function (req, res) {
      try {
        // Proteção contra campos de upload não utilizados
        req.file('fotos').upload({}, () => {});
    ...
    delete: async function (req, res) {
      try {
        ...
        // Remove fotos e categorias associadas antes
        await ReceitaFoto.destroy({ receita: id });
        await ReceitaCategorias.destroy({ receita: id });

        const receitaRemovida = await Receita.destroyOne({ id });
        if (!receitaRemovida) {
          return res.status(404).json({ erro: 'Receita não encontrada' });
        }
